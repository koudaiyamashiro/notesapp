import { BarChart3, Building2, Map, Smartphone, Sparkles, Target, TrendingUp } from 'lucide-react'
import SectionReveal from './SectionReveal.jsx'

const features = [
  {
    title: '市場価値スコア',
    description: '経験・スキル・市場需要を統合して、現在地と伸びしろを可視化します。',
    icon: BarChart3,
  },
  {
    title: '業界・職種の適性提案',
  ...
  {
    title: 'キャリアロードマップ',
    description: '1年、3年、5年の意思決定と必要アクションを提示します。',
    icon: Map,
  },
]
...
function FeatureCard({ feature, index }) {
  ...
  return (
    <article className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,0.05)] transition duration-300 hover:-translate-y-1 hover:border-sky-200 hover:shadow-[0_18px_55px_rgba(15,23,42,0.08)]">
...
function ProductMockup() {
  return (
    <div className="relative h-[560px] overflow-hidden rounded-[30px] border border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f7fbff)] shadow-[0_26px_84px_rgba(15,23,42,0.08)] sm:h-[620px] lg:h-[680px]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_12%,rgba(14,165,233,0.12),transparent_28%),radial-gradient(circle_at_80%_18%,rgba(79,123,255,0.12),transparent_26%)]" />
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-[0_16px_45px_rgba(15,23,42,0.05)]">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">市場価値スコア</p>
              ...
              <div className="mt-3 space-y-2.5">
                {[
                  ...
                  <div key={label} className="flex items-start gap-3">
                    <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#EAF6FF] text-[11px] font-semibold text-sky-700">{index + 1}</div>
                    <div>
...
export default function FeatureSection() {
  return (
    <section id="feature" className="bg-white py-10 sm:py-12">
        ...
        <SectionReveal>
          <div className="overflow-hidden rounded-[32px] border border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f8fbff)] p-4 shadow-[0_26px_90px_rgba(15,23,42,0.08)] sm:p-5">
            <div className="grid gap-5 lg:grid-cols-[0.92fr_1.08fr] lg:items-start">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 xl:pt-2">
                {features.map((feature, index) => (
                  <FeatureCard key={feature.title} feature={feature} index={index} />
                ))}
              </div>
